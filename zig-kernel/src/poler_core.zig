...
//      v6 FIX:  ARX-box с mixed_key + safe_key — полный key-dependent diffusion
//
//   2. Φ(x): ARX-box вместо полиномиальной конструкции.
//      Старый: x³ ⊕ x ⊕ 1 — уязвимо к алгебраическим атакам в GF(2)
//      v4:     rotl(x³, 13) ⊕ rotl(x, 7) ⊕ 1 — ротация ломает структуру
//      v6 FIX: ARX-box (add-rotate-xor) — мощнейшее перемешивание
//
//   3. ⊗_ε деформация: УБРАН AND (a∧b), который терял биты; XOR → add.
//      Старый: ε·((a∧b) ⊕ Φ(a⊕b)) — AND зануляет биты при a_i=b_i=0
//      v4:     ε·(rotl(a,5) ⊕ rotl(b,7) ⊕ Φ(a⊕b)) — ротация сохраняет все биты
//      v6 FIX: base_product +% epsilon_term — добавление вместо XOR
//
...
/// Нелинейная перестановка Φ(x) — v6 ARX-BOX
///
/// v6: ARX-box (Add-Rotate-Xor) вместо полиномиальной конструкции.
///
...
///
/// Решение v6: полноценный ARX-box с золотым сечением.
/// y = x +% 0x9E3779B9 — добавление разрушает XOR-линейность
/// rotl + xor-shift + multiply — каскадное перемешивание
/// Константа 0x517CC1B7 — нечётный множитель (биекция mod 2^32)
...
/// Деформированное тензорное произведение a ⊗_ε b — v6
/// a ⊗_ε b = (a · b) +% (ε · (rotl(a,5) ⊕ rotl(b,7) ⊕ Φ(a ⊕ b)))
...
pub fn nilpotentOperator(y: u32, key: u32, epsilon: u32) u32 {
    ...
    x ^= safe_key;
    x *%= safe_key;
    x +%= epsilon *% rotl(u32, safe_key, 7);
    x = phi(x);
    x *%= 0x9E3779B9;
    x +%= rotl(u32, safe_key ^ epsilon, 13);
    return rotl(u32, x, 13);
}
